...
def make_arena(arena_size=10, wall_height=1):
    ...
    # Wall Shape (Horizontal)
    wall_collision_shape_h = p.createCollisionShape(shapeType=p.GEOM_BOX, halfExtents=[arena_size/2, wall_thickness/2, wall_height/2])
    wall_visual_shape_h = p.createVisualShape(shapeType=p.GEOM_BOX, halfExtents=[arena_size/2, wall_thickness/2, wall_height/2], rgbaColor=[0.7, 0.7, 0.7, 1])

    # Create two horizontal walls
    p.createMultiBody(baseMass=0, baseCollisionShapeIndex=wall_collision_shape_h, baseVisualShapeIndex=wall_visual_shape_h, basePosition=[0, arena_size/2, wall_height/2])
    p.createMultiBody(baseMass=0, baseCollisionShapeIndex=wall_collision_shape_h, baseVisualShapeIndex=wall_visual_shape_h, basePosition=[0, -arena_size/2, wall_height/2])

    # Wall Shape (Vertical)
    wall_collision_shape_v = p.createCollisionShape(shapeType=p.GEOM_BOX, halfExtents=[wall_thickness/2, arena_size/2, wall_height/2])
    wall_visual_shape_v = p.createVisualShape(shapeType=p.GEOM_BOX, halfExtents=[wall_thickness/2, arena_size/2, wall_height/2], rgbaColor=[0.7, 0.7, 0.7, 1])

    # Create two vertical walls
    p.createMultiBody(baseMass=0, baseCollisionShapeIndex=wall_collision_shape_v, baseVisualShapeIndex=wall_visual_shape_v, basePosition=[arena_size/2, 0, wall_height/2])
    p.createMultiBody(baseMass=0, baseCollisionShapeIndex=wall_collision_shape_v, baseVisualShapeIndex=wall_visual_shape_v, basePosition=[-arena_size/2, 0, wall_height/2])
...
try:
    ...
    print("Warning: gaussian_pyramid.urdf not found.")
